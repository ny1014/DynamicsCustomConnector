...
class SalesforceRecordHandler(RecordHandler):
    ...
    def query_data(self, request: requests.QueryDataRequest) -> responses.QueryDataResponse:
        ...
        secret = secrets_manager.get_secret_value(SecretId= requests.connector_context.credentials.secret_arn)
        ...
        LOGGER.error(f'hey!test list_contacts = {list_contacts}')
...
# query_object
        ...
        return responses.QueryDataResponse(is_success=True,
                                           records=parse_query_response(list_contacts))
